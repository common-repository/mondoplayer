...
var media = (function () {
	...
	var updateHtml = function (html, data, updateAll) {
		console.log("updateHtml: " + JSON.stringify(data));
		var writer = global$7();
		var isEphoxEmbed = Cell(false);
		var sourceCount = 0;
		var hasImage;
		...
		text: function (text, raw) {
			...
			var intrinsic = "intrinsic-ignore";
			if (data.intrinsicscale) {
				intrinsic = "";
			}
			if (isEphoxEmbed.get()) ; else if (has(attrs.map, 'data-ephox-embed-iri')) {
			isEphoxEmbed.set(true);
	...
	var getIframeHtml = function (data) {
		var allowFullscreen = data.allowFullscreen ? ' allowFullscreen="1"' : '';
  		var intrinsicscale = ' class=""';
		if (! data.intrinsicscale) {
			intrinsicscale = ' class="intrinsic-ignore"';
		}

		return '<iframe src="' + data.source1 + '" width="' + data.width + '" height="' + data.height + '"' + intrinsicscale + allowFullscreen + '></iframe>';
	};
	var getFlashHtml = function (data) {
		...
		var html = '<object data="' + data.source1 + '" width="' + data.width + '" height="' + data.height + '" type="application/x-shockwave-flash"' + intrinsicscale + '>';
		if (data.poster) {
	...
	var getAudioHtml = function (data, audioTemplateCallback) {
		...
		return '<audio controls="controls" src="' + data.source1 + '"' + intrinsicscale + '>' + (data.source2 ? '\n<source src="' + data.source2 + '"' + (data.source2mime ? ' type="' + data.source2mime + '"' : '') + ' />\n' : '') + '</audio>';
		}
	};
	var getVideoHtml = function (data, videoTemplateCallback) {
		...
		return '<video width="' + data.width + '" height="' + data.height + '"' + (data.poster ? ' poster="' + data.poster + '"' : '') + ' controls="controls"' + intrinsicscale + '>\n' + '<source src="' + data.source1 + '"' + (data.source1mime ? ' type="' + data.source1mime + '"' : '') + ' />\n' + (data.source2 ? '<source src="' + data.source2 + '"' + (data.source2mime ? ' type="' + data.source2mime + '"' : '') + ' />\n' : '') + '</video>';
		}
	};
	...
	var createUi = function (onChange) {
		...
		items: [
			...
			checked: false,
			onclick: recalcSize,
			text: 'Expand to fill available space'
			}
